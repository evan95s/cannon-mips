...
func (s *stateObject) GetCommittedState(key common.Hash) common.Hash {
	// fmt.Println("call GetCommittedState")
	...
	// fmt.Println("GetCommittedState 1")
	...
	// fmt.Println("GetCommittedState 2")
	if s.db.snap != nil {
		start := time.Now()
	...
	// fmt.Println("GetCommittedState 3")
	// If the snapshot is unavailable or reading from it fails, load from the database.
	if s.db.snap == nil || err != nil {
		start := time.Now()
		// fmt.Println("GetCommittedState 3.1")
		// seems no difference???
		// oracle.PrefetchStorage(oracle.GetBlockNumber(), s.address, key, trie.GenPossibleShortNodePreimage)
		oracle.PrefetchStorage(oracle.GetBlockNumber(), s.address, key, nil)
		tr, err := s.getTrie()
		if err != nil {
			fmt.Println("get trie err:", err)
			s.db.setError(err)
			return common.Hash{}
		}
		// fmt.Println("GetCommittedState 3.2")
	...
	// fmt.Println("GetCommittedState 4")
...
func (s *stateObject) updateTrie() (Trie, error) {
	...
	for _, key := range deletions {
		oracle.PrefetchStorage(big.NewInt(oracle.GetBlockNumber().Int64()+1), s.address, key, trie.GenPossibleShortNodePreimage)
