...
import (
	...
	"github.com/ethereum/go-ethereum/wrapper"
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		oracle.SetBlockNumber(int64(blockNumber))
	...
	// get inputs
	fmt.Println("input hash is ", oracle.InputHash())
	inputBytes := oracle.Preimage(oracle.InputHash())
	fmt.Println("input bytes is ", inputBytes)
	...
	db := wrapper.NewDB(oracle.GetMemoryDB())
	fmt.Println("000")
	...
	bc, err := core.NewBlockChain(db, nil, core.DefaultGenesisBlock(), nil, &ethash.Ethash{}, vmconfig, nil, nil)
	if err != nil {
	...
	fmt.Println("tx hash is ", newheader.TxHash)
	statedb, err := state.New(parent.Root, wrapper.NewStateDB(parent, state.NewDatabaseWithNodeDB(db, trieDb), trieDb, db), nil)
	if err != nil {
