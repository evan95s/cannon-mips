...
func NewTrieDB(db ethdb.Database) database.Database {
	return &TrieDB{
		inner: triedb.NewDatabase(db, &triedb.Config{Preimages: true}),
	}
}

func (t *TrieDB) InsertPreimage(preimages map[common.Hash][]byte) {
	t.inner.InsertPreimage(preimages)
}

func (t *TrieDB) Preimage(hash common.Hash) []byte {
	return t.inner.Preimage(hash)
}

func (t *TrieDB) Reader(blockRoot common.Hash) (database.Reader, error) {
	return t.inner.Reader(blockRoot)
}
