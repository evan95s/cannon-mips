...
func New(id *ID, db database.Database) (*Trie, error) {
	...
	fmt.Println("set root here", id.Root)
	if id.Root != (common.Hash{}) && id.Root != types.EmptyRootHash {
		fmt.Println("ok root here", id.Root)
...
func (t *Trie) Get(key []byte) ([]byte, error) {
	// fmt.Println("try get is here with key", fmt.Sprintf("%x", key))
	// Short circuit if the trie is already committed and not usable.
	if t.committed {
		return nil, ErrCommitted
	}
...
func (t *Trie) get(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error) {
	...
	case valueNode:
		// fmt.Println("trie is value node")
		return n, n, false, nil
	case *shortNode:
		// fmt.Println("trie is short node")
		...
		// fmt.Println("return value with ", value)
		return value, n, didResolve, err
	case *fullNode:
		// fmt.Println("trie is full node")
...
func (t *Trie) Hash() common.Hash {
	...
	t.root = cached
	ret := common.BytesToHash(hash.(hashNode))
	// fmt.Println("[Hash]update root to ", ret)
	return ret
}
